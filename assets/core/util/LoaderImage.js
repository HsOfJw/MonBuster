...
function releaseNodeRes() {
    ...
    let texturesInCache = deepCloneObj(cc.loader._cache);

    let release_key = [];

    for (let asset in texturesInCache) {
...
function releaseStaticRes(tag) {
    ...
    let dependAssets = cc.director.getScene().dependAssets;
